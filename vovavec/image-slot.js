  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      this._img.addEventListener('load', () => {
        ...
        if (w && h) {
          this.dispatchEvent(new CustomEvent('slot-image-loaded', {
            bubbles: true, composed: true,
            detail: { id: this.id, naturalW: w, naturalH: h },
          }));
        }
      });
    ...
    // Build a static light-DOM mirror of the slot's current visual state for
    // html-to-image export. The library's shadow DOM serialization mangles
    // :host() selectors and inline images, so we hand it a plain div+img the
    // shape, radius/clip, and crop already applied. Returns null for empty
    // slots so the export can simply omit them.
    __exportClone() {
      const url = this._img && this._img.getAttribute('src');
      if (!url) return null;
      ...
      wrapper.setAttribute('data-export-clone-of', this.id || '');
      wrapper.style.cssText = 'position:relative;overflow:hidden;background:rgba(0,0,0,.04);';
      if (mask) wrapper.style.clipPath = mask;
      else if (radius) wrapper.style.borderRadius = radius;

      const img = document.createElement('img');
      img.src = url;
      const is = this._img.style;
      img.style.cssText = 'position:absolute;max-width:none;transform:translate(-50%,-50%);user-select:none;';
      img.style.width = is.width || '100%';
      img.style.height = is.height || '100%';
      img.style.left = is.left || '50%';
      img.style.top = is.top || '50%';
      img.style.objectFit = is.objectFit || '';
      img.style.objectPosition = is.objectPosition || '';
    ...
    async _ingest(file) {
      ...
      try {
        ...
        const w = (this.clientWidth || this.offsetWidth || MAX_DIM) * dpr;
